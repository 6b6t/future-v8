...
public final class GuiTabHandler {
    ...
    public void drawGui(int x, int y) {
        ...
        for (int i = 0; i < this.tabs.size(); ++i) {
            ...
            if (this.selectedTab != i) continue;
            RenderMethods.drawGradientBorderedRectReliant(x, (float)(i * 12 + y + transitionTop) - 0.3f, (float)(x + guiWidth) - 2.2f, (float)(i + (y + 12 + i * 11) + transitionBottom) + 0.3f, 1.5f, -2013265920, -1141161406, -1141161406);
        }
        int yOff = y + 2;
        for (int index = 0; index < this.tabs.size(); ++index) {
            GuiTab tab = this.tabs.get(index);
            this.mc.fontRenderer.drawStringWithShadow(tab.getLabel(), x + 2, yOff, -197380);
